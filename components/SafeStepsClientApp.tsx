...
export default function SafeStepsClientApp() {
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      if (error) console.error("Failed to fetch premium status:", error)
        ...
        <Tabs defaultValue="home" value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
          ...
          <div className="fixed bottom-0 left-0 right-0 border-t bg-white">
            <TabsList className="w-full h-16 grid grid-cols-4 bg-transparent">
              <TabsTrigger value="home" className="flex flex-col items-center justify-center data-[state=active]:bg-blue-50 rounded-none">
                <Home className="h-5 w-5" />
                <span className="text-xs mt-1">Home</span>
              </TabsTrigger>
              <TabsTrigger value="contacts" className="flex flex-col items-center justify-center data-[state=active]:bg-blue-50 rounded-none">
                <Users className="h-5 w-5" />
                <span className="text-xs mt-1">Contacts</span>
              </TabsTrigger>
              <TabsTrigger value="history" className="flex flex-col items-center justify-center data-[state=active]:bg-blue-50 rounded-none">
                <Clock className="h-5 w-5" />
                <span className="text-xs mt-1">History</span>
              </TabsTrigger>
              <TabsTrigger value="settings" className="flex flex-col items-center justify-center data-[state=active]:bg-blue-50 rounded-none">
                <Settings className="h-5 w-5" />
                <span className="text-xs mt-1">Settings</span>
              </TabsTrigger>
              <TabsTrigger value="alerts" className="flex flex-col items-center justify-center data-[state=active]:bg-red-50 rounded-none">
